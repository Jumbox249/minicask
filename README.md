...
- **Restoring a snapshot holds the lock.** A follower that is sent one rewrites its store under the node's lock, streaming, but while consensus waits. It is not serving anything until it is done, so the cost lands on a node that is behind anyway.
...
259 tests, including the three that matter:
